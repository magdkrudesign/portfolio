...
export default function ProjectPage({ params }: Props) {
    ...
    <>
      ...
      <div style={{ display: "flex", flexDirection: "column" }}>
        {[
          ...
          >
            <span style={{ fontSize: 16, fontWeight: 500, letterSpacing: "-0.03em", lineHeight: "1.4em", flex: 1 }}>
              {value}
            </span>
            <span style={{ fontSize: 14, fontWeight: 500, letterSpacing: "-0.03em", lineHeight: "1.4em", color: "var(--color-gray)", flexShrink: 0 }}>
              {label}
            </span>
          </div>
        ))}
  ...
  return (
    <PageLayout left={leftCol} right={rightCol}>
      ...
      {/* ← All work / Next → navigation */}
      <div style={{ marginTop: 80, paddingTop: 24, borderTop: "1px solid var(--color-light-gray)",
          display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <Link href="/work" className="text-14" style={{ color: "var(--color-gray)", textDecoration: "none" }}>
          ← All work
        </Link>
        <Link href={`/work/${next.slug}`} className="text-14" style={{ color: "var(--color-black)", textDecoration: "none" }}>
          {next.title} →
        </Link>
      </div>
